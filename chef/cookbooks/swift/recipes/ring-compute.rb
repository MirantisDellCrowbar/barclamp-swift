...
nodes.each { |node|    
  ...
  disks.each {|uuid,disk|
    ...
    z_c,w_c = Swift::Evaluator.eval_with_params(disk_assign_expr, node(), :ring=> "object", :disk=>disk, :target_node=>n)
    z_a,w_a = Swift::Evaluator.eval_with_params(disk_assign_expr, node(), :ring=> "object", :disk=>disk, :target_node=>n)
